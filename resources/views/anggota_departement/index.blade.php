                ...
                <tbody id="divisi-table-body">
                    @foreach ($anggota_departements as $anggota_departement)
                        <tr>
                            ...
                            <td class="py-2 px-4 border-b border-r">
                                {{ $anggota_departement->singkatan }}
                            </td>
                            <td class="py-2 px-4 border-b border-r">
                                {{ $anggota_departement->deskripsi }}
                            </td>
                            @auth
                                <td class="py-2 px-4 border-b border-r ">
                                    <div class="flex">
                                        ...
                                        <form action="/anggota_departement-delete/{{ $anggota_departement->id }}"
                                            method="POST" class="inline-block" id="formDelete-{{ $loop->iteration }}">
                                            @method('delete')
                                            @csrf
